...
object KrdResourceSpec : Spek({
    ...
    describe("Namespaced") {
        ...
        val client by memoized(CachingMode.SCOPE) {
            ...
            kubernetesClient(namespace = testNamespace)
        }

        beforeGroup {
            Launcher(MyKoperator(client), Action.INSTALL, client).run()
        }

        afterGroup {
            Launcher(MyKoperator(client), Action.UNINSTALL, client).run()
        }

        afterEachGroup {
            Launcher(MyKoperator(client), Action.RESET, client).run()
        }
